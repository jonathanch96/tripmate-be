...
func (a *adapterGormPostgresql) BulkCreate(ctx context.Context, expenseID uuid.UUID, rows []domainexpense.Split) error {
	if len(rows) == 0 {
		return nil
	}
	models := make([]Split, len(rows))
...
func (a *adapterGormPostgresql) ListByExpenseIDs(ctx context.Context, ids []uuid.UUID) (map[uuid.UUID][]domainexpense.Split, error) {
	result := make(map[uuid.UUID][]domainexpense.Split, len(ids))
	if len(ids) == 0 {
		return result, nil
	}
	var models []Split
...
func (a *adapterGormPostgresql) DeleteByExpenseID(ctx context.Context, expenseID uuid.UUID) error {
	return appdb.FromContext(ctx, a.db).WithContext(ctx).Where("expense_id = ?", expenseID).Delete(&Split{}).Error
}
